...
export class DashboardService {
  async getDashboardStats(userId: number): Promise<DashboardStats> {
    try {
      ...
      const currentMonthResult = await query<{ income: string; expense: string }>(
        `SELECT 
      ...
      const lastMonthResult = await query<{ income: string; expense: string }>(
        `SELECT 
          COALESCE(SUM(CASE WHEN type = 'income' THEN amount ELSE 0 END), 0) as income,
          COALESCE(SUM(CASE WHEN type = 'expense' THEN amount ELSE 0 END), 0) as expense
         FROM transactions
  ...
  async getMonthlyTrend(userId: number, months: number = 6): Promise<MonthlyData[]> {
    try {
      ...
      logger.error('❌ getMonthlyTrend error:', error);
      throw error;
    }
  }

  async getExpenseByCategory(userId: number, startDate?: string, endDate?: string): Promise<CategorySummary[]> {
    try {
      const currentDate = new Date();
      ...
      logger.error('❌ getExpenseByCategory error:', error);
      throw error;
    }
  }

  async getRecentTransactions(userId: number, limit: number = 5): Promise<RecentTransaction[]> {
    try {
