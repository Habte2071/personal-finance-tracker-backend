...
class DashboardService {
    async getDashboardStats(userId) {
        try {
            ...
            // Get total balance
            const balanceResult = await (0, database_1.query)('SELECT COALESCE(SUM(balance), 0) as total_balance FROM accounts WHERE user_id = ? AND is_active = true', [userId]);
            // Get current month income/expense
            const currentMonthResult = await (0, database_1.query)(`SELECT 
          COALESCE(SUM(CASE WHEN type = 'income' THEN amount ELSE 0 END), 0) as income,
          COALESCE(SUM(CASE WHEN type = 'expense' THEN amount ELSE 0 END), 0) as expense
         FROM transactions
         WHERE user_id = ? AND transaction_date >= ?`, [userId, currentMonthStart.toISOString().split('T')[0]]);
            // Get last month income/expense
            const lastMonthResult = await (0, database_1.query)(`SELECT 
          COALESCE(SUM(CASE WHEN type = 'income' THEN amount ELSE 0 END), 0) as income,
          COALESCE(SUM(CASE WHEN type = 'expense' THEN amount ELSE 0 END), 0) as expense
         FROM transactions
         WHERE user_id = ? AND transaction_date BETWEEN ? AND ?`, [userId, lastMonthStart.toISOString().split('T')[0], lastMonthEnd.toISOString().split('T')[0]]);
            ...
            config_1.default.error('❌ getDashboardStats error:', error);
            throw error;
        }
    }
    async getMonthlyTrend(userId, months = 6) {
        try {
         ...
         ORDER BY month ASC`, [userId, startDateStr]);
            ...
            config_1.default.error('❌ getMonthlyTrend error:', error);
            throw error;
        }
    }
    async getExpenseByCategory(userId, startDate, endDate) {
        try {
         ...
         ORDER BY total DESC`, [
            ...
            config_1.default.error('❌ getExpenseByCategory error:', error);
            throw error;
        }
    }
    async getRecentTransactions(userId, limit = 5) {
        try {
         ...
         LIMIT ?`, [userId, limit]);
            ...
            config_1.default.error('❌ getRecentTransactions error:', error);
            throw error;
        }
    }
}
